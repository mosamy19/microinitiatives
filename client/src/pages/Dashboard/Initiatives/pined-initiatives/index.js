...
const Pinedinitiatives = () => {
  ...
  let pined_initiatives =
    all_initiatives.length > 0 &&
    all_initiatives.filter((item) => item.pined === true);

  useEffect(() => {
    if (pined_initiatives.length > 0) {
      let temp = [];
      pined_initiatives.map((item, index) => {
        temp = [
          ...temp,
          {
            key: item._id,
...
export default Pinedinitiatives;
